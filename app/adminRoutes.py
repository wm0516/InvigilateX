...
def process_exam_row(row, slot_share_dt, slot_open_dt):
    ...
    if requested_capacity > available_capacity:
        ...
        return None, ''
    flash(f"Start: {start_dt}, End: {end_dt}, CourseCode: {str(row.get('course code')).upper()}, Venue: {[venue]}, Capacity: {[requested_capacity]}, Open: {slot_share_dt}, Close: {slot_open_dt}", "success")
