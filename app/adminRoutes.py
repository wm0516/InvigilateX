...
def process_exam_row(row, slot_share_dt, slot_open_dt):
    ...
    if requested_capacity > available_capacity:
        ...
        return None, ''
    flash(f"Start: {start_dt}, End: {end_dt}, CourseCode: {str(row.get('course code/section')).upper()}, Venue: {[venue]}, Capacity: {[requested_capacity]}, Open: {slot_share_dt}, Close: {slot_open_dt}", "success")
...
def adjust_exam(exam, new_start, new_end, new_venues, new_students):
    try:
        ...
        for r in reports:
            for att in r.attendances:
                inv = att.invigilator
                if inv:
                    inv.userPendingCumulativeHours = max(0.0, (inv.userPendingCumulativeHours or 0) - old_hours)
                db.session.delete(att)
            db.session.delete(r)
        VenueExam.query.filter_by(examId=exam.examId).delete()
        db.session.flush()

        # Recreate using upload logic
        create_exam_and_related(
            ...
            student_list=new_students,
            open_dt=datetime.now() + timedelta(hours=8),
            close_dt=datetime.now() + timedelta(days=14)
        )
        db.session.commit()
    except Exception as e:
        db.session.rollback()
